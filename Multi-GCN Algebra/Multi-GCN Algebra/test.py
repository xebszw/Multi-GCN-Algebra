...
def split_ids(ids, folds=10):
    ...
    if args.dataset == 'COLORS-3':
        ...
        stride = int(np.ceil(n / float(folds))) #每个group的样本数(这里将步长略微取大了，为了保证没有样本会被遗漏)
...
if not args.torch_geom:
    ...
    class DataReader():
        ...
        def __init__(self,
                     ...
                     rnd_state=None,
                     use_cont_node_attr=False, # 把该变量调整为True 
...
def collate_batch(batch):
    ...
    graph_support = torch.zeros(B, N_nodes_max) #把这个看成掩码mask。方便运算
    A = torch.zeros(B, N_nodes_max, N_nodes_max) #这是一个batch的所有邻接矩阵
    x = torch.zeros(B, N_nodes_max, C) #这是batch中每个节点的初始向量
    for b in range(B):
        if args.torch_geom:
